...
import {
  ...
  fetchCallAuditProgressThunk,
  setCallAuditFiltersAct,
} from "../../redux/customer-call-audit.slice"

export default function CallAuditProgressFiltersCP() {
  const dispatch = useAppDispatch()
  const { filters, loadingProgress } = useAppSelector((state) => state.customerCallAudit)
    ...
    <Paper variant="outlined" sx={{ p: 2, mb: 2 }}>
      <Stack direction={{ xs: "column", md: "row" }} spacing={2} alignItems={{ md: "center" }} useFlexGap flexWrap="wrap">
        ...
        <Button
          ...
          onClick={runSearch}
          disabled={loadingProgress || filters.month === ""}
          sx={{ cursor: "pointer" }}
        >
          {s.search}
        </Button>
